...
<body>
    ...
    <div class="text">
        <div class="container mt-4">
            <h1 class="mb-4">Yugioh Card Info</h1>

            <div class="search-container">
                <form action="{{ route('cardinfo') }}" method="GET" class="form-group">
                    <select name="type" class="form-control">
                        <option value="" {{ request('type') ? '' : 'selected' }}>Chọn loại thẻ</option>
                        <option value="SC" {{ request('type') == 'Spell Card' ? 'selected' : '' }}>Spell Card</option>
                        <option value="MC" {{ request('type') == 'Monster' ? 'selected' : '' }}>Monster</option>
                        <option value="TC" {{ request('type') == 'Trap Card' ? 'selected' : '' }}>Trap Card</option>
                    </select>

                    <input type="text" name="name" class="form-control" value="{{ request('name') }}" placeholder="Nhập tên thẻ cần tìm" autocomplete="off">
            ...
            @if(request('name'))
            <p>Kết quả tìm kiếm cho "{{ request('name') }}": {{ $totalItems }} thẻ</p>
            @endif
